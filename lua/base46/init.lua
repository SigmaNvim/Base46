...
M.get_theme_tb = function(type)
  local default_path = "base46.themes." .. g.sigma_theme
  local user_path = "custom.themes." .. g.sigma_theme
...
M.toggle_theme = function()
  ...
  if g.sigma_theme == theme1 or g.sigma_theme == theme2 then
  ...
  if g.sigma_theme == theme1 then
    g.sigma_theme = theme2

    require("sigma").reload_theme()
    require("sigma").change_theme(theme1, theme2)
  elseif g.sigma_theme == theme2 then
    g.sigma_theme = theme1

    require("sigma").reload_theme()
    require("sigma").change_theme(theme2, theme1)
  else
    vim.notify "Set your current theme to one of those mentioned in the theme_toggle table (sigmarc)"
  end
end

M.toggle_transparency = function()
  local transparency_status = config.ui.transparency
  local write_data = require("sigma").write_data

  local function save_sigmarc_data()
